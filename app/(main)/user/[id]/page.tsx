    ...
    <div className="container mx-auto py-8 px-4">
      {isOwnProfile && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <p className="text-sm text-blue-700">You are viewing your public profile as others see it</p>
          </div>
          <Link href="/me">
      ...
      <div className="bg-white border rounded-lg p-6 mb-8 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-sm text-muted-foreground mb-3">@{displayUsername}</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mb-4">
              ...
              <div className="flex items-center gap-1 justify-center sm:justify-start">
                ...
                <p className="text-sm text-muted-foreground">Joined {new Date(userData.joinedDate).toLocaleDateString(undefined, { year: 'numeric', month: 'long' })}</p>
              </div>
            </div>
            ...
            {(userData.socialLinks?.facebook || userData.socialLinks?.twitter || userData.socialLinks?.instagram) && (
              <div className="flex items-center gap-3 justify-center sm:justify-start mb-4">
                ...
                {userData.socialLinks?.facebook && (
                  <Link href={userData.socialLinks.facebook} target="_blank" className="text-blue-600 hover:text-blue-800 transition-colors">
                ...
                {userData.socialLinks?.twitter && (
                  <Link href={userData.socialLinks.twitter} target="_blank" className="text-blue-400 hover:text-blue-600 transition-colors">
                ...
                {userData.socialLinks?.instagram && (
                  <Link href={userData.socialLinks.instagram} target="_blank" className="text-pink-600 hover:text-pink-800 transition-colors">
                    <div className="p-2 rounded-full bg-pink-50 hover:bg-pink-100 transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          ...
          <div className="flex flex-row sm:flex-col gap-4 mt-4 sm:mt-0 text-center">
            <div className="bg-secondary/10 rounded-lg px-4 py-3 flex-1">
              <p className="text-xl font-bold">{userData.readingStats.booksRead || 0}</p>
              <p className="text-xs text-muted-foreground">Books Read</p>
            </div>
            <div className="bg-secondary/10 rounded-lg px-4 py-3 flex-1">
              <p className="text-xl font-bold">{(userData as any).authoredBooks?.length || 0}</p>
              <p className="text-xs text-muted-foreground">Books Created</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-lg p-6 shadow-sm mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">Books by {userData.name}</h2>
